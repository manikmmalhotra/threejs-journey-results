...
vec3 colorA = vec3(0.447, 0.874, 0.458);
vec3 colorB = vec3(0.941, 0, 0.098);

void main() {
    vec3 color = vec3(0.0);

    float pct =  1.0 - pow(1.0 - uTime, 5.0);
...
//    float strength = random(gridUv);
    ...
    // float strength = angle;
     ...
     // Pattern 44
    // float angle = atan(vUv.x - 0.5, vUv.y - 0.5) / (PI * 2.0) + 0.3;
    ...
    // Black annd White
    // Freestyle Pattern
    // float strength =step(0.1, mod(0.3, abs(cnoise(vUv * 10.0))));
